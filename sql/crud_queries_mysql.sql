...
-- CREATE
INSERT IGNORE INTO retailers (retailer_name) VALUES ('Demo Electronics');
...
SELECT p.product_id, r.retailer_id, 'DEMO-M331', 859.00,
       1295.00, 4.3, 25, TRUE, '2026-05-21'
FROM products p
JOIN retailers r ON r.retailer_name = 'Demo Electronics'
...
SELECT p.product_name, r.retailer_name, po.listed_price_inr,
       ROUND((po.original_price_inr - po.listed_price_inr)
             * 100 / po.original_price_inr, 2) AS discount_percent
FROM price_observations po
JOIN products p ON p.product_id = po.product_id
JOIN retailers r ON r.retailer_id = po.retailer_id
WHERE p.product_key = 'LOG-M331' AND po.in_stock = TRUE
ORDER BY po.listed_price_inr;

-- UPDATE
UPDATE price_observations
SET listed_price_inr = 829.00, captured_date = '2026-05-22'
...
-- PRICE COMPARISON REPORT (MySQL 8.0+)
WITH ranked AS (
    SELECT p.product_name, r.retailer_name, po.listed_price_inr,
           ROW_NUMBER() OVER (
...
SELECT product_name, retailer_name AS cheapest_retailer,
       listed_price_inr AS lowest_price, highest_price,
